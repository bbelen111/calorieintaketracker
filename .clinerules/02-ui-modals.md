...
39. **The carousel loops with per-slide wrapped transforms — keep React out of the slide styles:** `useSwipeableScreens` renders each screen once and writes every slide `transform` + `--screen-drag-progress` imperatively (`commitPosition`), on mount via the `getSlideProps(index)` ref callbacks and per frame during drags/settles. Never author a slide `transform`/`transition` from React props and never reintroduce a CSS settle transition: that combination previously stranded an inline `transition: none` (e.g. after a vertical scroll, which abandons the gesture in the axis-lock branch) and made the next tab tap snap instead of slide. Keep the loop math in `utils/visuals/carouselLoop.js` (`resolveSlideOffsets`, `resolveSettleTarget`, `alignPositionToReference`, `resolveShortestScreenDelta`, `getLoopSlideFadeSide`, `carouselEase`) — no duplicated wrap/settle arithmetic in components, and no cloned slides/ghost cells/timers.
