...
**Daily Ledger presentation helpers:** `utils/calculations/dayLedgerPresentation.js` is the canonical read-only display-model source for the Logbook Daily Ledger frontend (`DayLedgerListModal` + `DayLedgerModal`). Exports: `isValidDaySnapshot` (structural gate — valid canonical date plus finite `tdee`/`intake`; malformed records are unavailable days, never zeros), `buildDayLedgerRows` (ordered energy-composition rows; NEAT derived/clamped from `baselineTdee`), `buildDaySnapshotPreview` (compact preview incl. clamped `cardioBurn`/`trainingBurn`, positive-only `barSegments`, full `rawRows`), `summarizeMonthSnapshots` (month rollup: avg energy/balance/steps, kind-day counts, `estimatedWeightChangeKg` ÷7700, plus measurement averages computed only from canonical tracker entries dated inside the month — never borrowed across months), `getMeasurementForDate` (exact-date tracker lookup; absent/garbage → `null`, never zero-filled), `getAdjacentTrackedDates`, `formatSignedKcal`, and `DAY_LEDGER_BALANCE_META`. Sign convention matches the snapshot `deficit` field (positive = deficit). Do not duplicate this logic in UI components.
