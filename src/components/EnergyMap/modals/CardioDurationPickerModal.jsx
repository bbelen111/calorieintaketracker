...
const convertMinutesToParts = (totalMinutes) => {
  if (!Number.isFinite(totalMinutes)) {
    return { hours: 0, minutes: 0 };
  }

  const cappedMinutes = Math.min(Math.max(totalMinutes, 0), MAX_HOURS * 60);
...
export const CardioDurationPickerModal = (props) => {
  ...
  const emitDurationChange = useCallback(
  ...
  useEffect(() => {
    ...
    const parts = convertMinutesToParts(minutes);
  ...
  const handleHoursChange = useCallback(
    (nextHours) => {
      ...
      emitDurationChange(clampedHours, nextMinutes);
    },
    [emitDurationChange]
  );
  ...
  const handleMinutesChange = useCallback(
    (nextMinutes) => {
      ...
      emitDurationChange(selectionRef.current.hours, sanitizedMinutes);
    },
    [emitDurationChange]
  );
