...
const getGoalClasses = (key, selected) => {
  ...
  if (!goal)
    return 'border-border/80 bg-surface-highlight text-foreground/80 hover:border-border';
  if (!selected)
    return 'border-border/80 bg-surface-highlight text-foreground/80 hover:border-border';
    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-lg border border-border/80 bg-surface-highlight px-3 py-1.5 text-xs font-semibold uppercase tracking-wide text-foreground transition-colors md:hover:border-amber-400 md:hover:text-foreground focus-ring pressable"
        >
          <Star size={14} />
          Templates
        </button>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-surface-highlight border border-border/80 rounded-lg px-4 py-3 text-foreground placeholder-muted focus:outline-none focus:ring-2 focus:ring-blue-500"
            maxLength={50}
          />
        </div>
      ...
      <div className="flex gap-3 mt-6">
        <button
          type="button"
          onClick={onCancel}
          className="flex-1 px-4 py-3 bg-surface-highlight md:hover:bg-surface-highlight/90 text-foreground rounded-lg font-semibold transition-all focus-ring press-feedback"
        >
          Cancel
        </button>
        <button
