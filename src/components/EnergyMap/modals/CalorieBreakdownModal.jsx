...
export const CalorieBreakdownModal = ({
  ...
  const formattedDifference = difference !== null && difference !== undefined ? difference : null;

  return (
    <ModalShell isOpen={isOpen} isClosing={isClosing}>
      <div className="p-5 md:p-6 w-full max-w-md">
        <div className="flex items-start justify-between gap-4 mb-4">
          <div>
            <h3 className="text-white font-bold text-xl">Calorie Breakdown</h3>
            <p className="text-slate-400 text-sm mt-1">
              {stepRange} steps • {selectedDay === 'training' ? 'Training' : 'Rest'} day • {goal.label}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-3 text-sm">
          <div className="flex items-center justify-between bg-slate-700/40 rounded-lg px-4 py-3">
            ...
            <span className="text-white font-semibold">{breakdown.bmr.toLocaleString()} cal</span>
          </div>
          <div className="flex items-center justify-between bg-slate-700/40 rounded-lg px-4 py-3">
          ...
          <div className="flex items-center justify-between bg-slate-700/40 rounded-lg px-4 py-3">
            ...
            <span className="text-white font-semibold">{breakdown.stepCalories.toLocaleString()} cal</span>
          </div>
          <div className="flex items-center justify-between bg-slate-700/40 rounded-lg px-4 py-3">
            <span className="text-slate-400">Training</span>
            <span className="text-white font-semibold">{breakdown.trainingBurn.toLocaleString()} cal</span>
          </div>
          <div className="flex items-center justify-between bg-slate-700/40 rounded-lg px-4 py-3">
            <span className="text-slate-400">Cardio</span>
            <span className="text-white font-semibold">{breakdown.cardioBurn.toLocaleString()} cal</span>
          </div>
          <div className="border border-slate-600 rounded-lg px-4 py-3 flex items-center justify-between">
            ...
            <span className="text-white font-bold text-lg">{breakdown.total.toLocaleString()} cal</span>
          </div>
        </div>
