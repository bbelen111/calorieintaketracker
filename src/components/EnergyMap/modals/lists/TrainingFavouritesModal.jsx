...
const getTrainingLabel = (trainingTypes, type) =>
  trainingTypes?.[type]?.label ?? 'Custom Training';
...
const isSameSession = (current, favourite) => {
  ...
  if (
    (normalizedCurrent.type ?? 'trainingtype_1') !==
    (normalizedFavourite.type ?? 'trainingtype_1')
  ) {
    return false;
  }
...
export const TrainingFavouritesModal = ({
  isOpen,
  isClosing,
  favourites,
  trainingTypes,
  ...
  const {
    trainingTypes: storeTrainingTypes,
    trainingFavourites,
  ...
  const resolvedFavourites = favourites ?? trainingFavourites;
  ...
  const sortedFavourites = useMemo(() => {
    ...
    return resolvedFavourites
      ...
      .slice()
      .sort((a, b) => {
        const labelA = getTrainingLabel(
          resolvedTrainingTypes,
          a.type
        ).toLowerCase();
        const labelB = getTrainingLabel(
          resolvedTrainingTypes,
          b.type
        ).toLowerCase();
        if (labelA === labelB) {
  ...
  }, [resolvedTrainingTypes, resolvedFavourites]);
        ...
        >
          {hasFavourites ? (
                    ...
                    {typeof onDeleteFavourite === 'function' &&
                      favourite.id != null && (
                        <button
                          ...
                          aria-label="Delete favourite training session"
                        >
                          <Trash2 size={14} />
                        </button>
                      )}
            ...
            <div className="text-center text-muted text-sm py-10">
              No favourites yet. Add one to reuse your preferred training setup.
            </div>
          )}
        </div>
