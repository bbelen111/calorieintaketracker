...
84. **Health Connect step reads should prefer native defaults first:** `useHealthConnect.fetchSteps()` should let `Health.readSamples(...)` use its native `now - 1 day` / `now` defaults first, then fall back to an explicit window from `buildHealthConnectStepReadWindow()` if the default query fails. The Android plugin also has a chunked read fallback for problematic full-range queries. If every read still fails on a real device, return `null` and keep the app usable instead of surfacing a connection error. Keep the fallback rolling 24-hour window in the same helper module for recovery paths.
