...
**Crude AT pressure accumulator:** Crude mode is strictly goal-driven and deterministic — it never reads `intakeCalories`, energy balance, or weight logs. It evaluates a signed, asymmetric balance-pressure accumulator over a rolling window of up to `CRUDE_WINDOW_DAYS` (28) daily snapshots ending at the requested `dateKey`; each day's goal is read from `snapshot.selectedGoal ?? snapshot.goalAtSnapshot` (unknown records are skipped, never zero-filled). Transitions: `cut` drives pressure −1.0/day (resets to −1.0 when the accumulator is positive — leaving surplus); `surplus` recovers +2.0/day while negative, else accumulates +0.75/day; `maintenance` decays +0.25/day when negative and −0.50/day when positive. Pressure is clamped to ±28. Milestones: `pressure <= -3/-7/-14/-21` → −50/−100/−175/−250 kcal/day; `pressure >= 7/14/21` → +50/+100/+150 kcal/day; otherwise 0. Because crude evaluates the snapshot history rather than `goalDurationDays`, an isolated maintenance/refeed day decays the accumulated adaptation instead of resetting it to zero — and the `maintenance-goal` no-op guard applies to smart mode only. The store's `setSelectedGoal` refreshes today's snapshot (`goalAtSnapshot`) on an actual goal switch so same-day changes feed the accumulator immediately.
