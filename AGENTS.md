...
- **Looping (clone-free):** every screen renders exactly once and each slide carries its **own** transform: `offset_i(position) = wrapIntoHalf(i + 1 − position)` in slide widths and `transform_i = translateX(calc((offset_i − i) * 100% + 16px))`. `%` resolves against the slide's own box width (viewport − 32px), so geometry stays exact across resizes with no px measurement. The position is one continuous, unbounded value; a slide teleports by ±N only when it crosses ±N/2 (~2 slides off-screen), and normalizing a wrap (6 → 1, 0 → 5) rewrites byte-identical transforms — no clones, ghost cells, timers or blank peek slivers.
- **Settle is JavaScript, not CSS:** drag frames and settles are written imperatively by `useSwipeableScreens` with `requestAnimationFrame` + `carouselEase` (`CAROUSEL_SETTLE_MS = 350`, matching the old `cubic-bezier(0.22, 1, 0.36, 1)`). Slides carry **no** `transition` and React never authors a slide `transform` (`getSlideProps(index)` supplies only a stable ref), so a store-driven re-render can never fight a drag frame and a stranded inline `transition: none` can never make a tab tap snap.
...
- **Alpha masks:** only the wrapped neighbours of the current screen carry `.slide-fade-left` / `.slide-fade-right` (20px linear-gradient masks in `index.css`), resolved through `getLoopSlideFadeSide(index, currentScreen, screenTabs.length)` so the masks stay correct across the loop seam (screen 4 is the previous neighbour of screen 0).
...
- **Drag progress:** `commitPosition` publishes `--screen-drag-progress` (float 0–4, clamped) on `:root` imperatively on every transform commit. The tab-bar circle and header dots read it via `calc(...)`, so they track the finger frame-by-frame with **zero React re-renders** (preserves the hook's no-state-churn design). During a wrap drag the affordance UI pins at the end it is leaving, then glides to the wrapped tab once the settle normalizes the position.
...
- A gesture that locks to the vertical axis abandons the swipe and re-syncs the carousel to its rest screen (`applyNormalizedPosition`) — it never touches slide styles or transitions.
- Viewport resize updates are `ResizeObserver`-driven but `requestAnimationFrame`-throttled with equality guards to avoid resize-state churn (transforms are `%`-based, so a resize needs no transform rewrite).
...
96. **Peek fades are alpha masks on wrapped neighbours only:** apply `.slide-fade-left/right` only to the wrapped neighbours via `getLoopSlideFadeSide(index, currentScreen, screenTabs.length)`, and never replace the masks with gradient overlay strips (overlays tint the faded pixels instead of letting the page gradient show through).
97. **`--screen-drag-progress` is the drag-sync channel:** published imperatively on `:root` by `useSwipeableScreens.commitPosition` and consumed via `calc(...)` by the tab-bar circle and header dots. Do not replace it with React state (re-renders per drag frame) and do not introduce per-button conditional remounts for the moving pill/circle/dot.
...
99. **The carousel loops with per-slide wrapped transforms — keep React out of the slide styles:** `useSwipeableScreens` renders each screen once and writes every slide `transform` + the progress variable imperatively (mount ref callbacks + per frame), so never author a slide `transform`/`transition` from React style props and never reintroduce a CSS settle transition. That mix previously stranded an inline `transition: none` — a vertical scroll abandons the gesture in the axis-lock branch without flipping `isSwiping`, so React never rewrote the transition and the next tab tap snapped instead of sliding. All loop math stays in `utils/visuals/carouselLoop.js` (`resolveSlideOffsets`, `resolveSettleTarget`, `alignPositionToReference`, `resolveShortestScreenDelta`, `getLoopSlideFadeSide`, `carouselEase`) with no cloned slides, ghost cells or normalization timers.
