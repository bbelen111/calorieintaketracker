...
84. **Health Connect step reads need a strict window helper:** `useHealthConnect.fetchSteps()` must use `buildHealthConnectStepReadWindow()` from `src/utils/healthConnectWindow.js` so `endDate` is always strictly after `startDate` before calling `Health.readSamples(...)`. Exact-midnight or clock-edge windows can otherwise trigger native `startTime must be before endTime` failures.
