...
- **1 lock:** horizontal `<input type="range">` slider (the feasible space is 1D — only the ratio between the two unlocked macros can vary). The slider track uses a two-color gradient matching the two unlocked macro accent colors, with end labels showing grams and percentages. The 1-lock slider path uses helpers from `macroRecommendations.js`: `getUnlockedMacroKeys`, `getUnlockedMacroRatio`, and `macroSplitFromUnlockedRatio`.
