...
- **Placement is fixed, never measured.** The card is `absolute top-2` inside a `relative` graph container, horizontally centred over the plot via `flex justify-center` and an inset that clears the y-axis column (`className="left-0 right-14"` for the 12-wide axes, `right-16` for Rolling's `w-14`). The wrapper is `pointer-events-none`, only the card captures pointers, and no inline `left`/`top` is ever written.
...
- **Interaction:** tap a point/bar/axis label to select (re-tapping the same slot in the day modes keeps the long-standing "tap again to edit/add" gesture, and the card body is the second, discoverable route); the card's X deselects; view-mode/window changes dismiss. Panning no longer dismisses anything. Empty slots are not selectable in Step/Rolling (honest "no data"), while Weight/Body Fat keep them selectable through the axis label so the add flow stays reachable.
- **Anatomy:** compact on purpose (the plot is the modal's `flex-1` remainder): muted date/month row, a `text-2xl` primary value, an inline metric row (`7d`/`14d`, Distance/Burned, Total, TDEE/Intake …), and — only when the modal has an edit/add flow — a muted `border-t` footer (`Tap to edit entry` / `Tap to add entry` + chevron) that makes the whole body a `<button>` (`pressable-card` + `focus-ring`, `ariaLabel` from the caller). Read-only variants (Step, 12m month selections, Rolling) render as a `role="status" aria-live="polite"` surface with no X-adjacent action, and the X is a **sibling** of the body, never a nested button.
...
- Covered by UI-tier specs: `common/TrackerSelectionCard.spec.jsx` (mount-always + crossfade, fixed slot with no inline geometry, pointer transparency, action/X separation, read-only variant) plus one per modal — `WeightTrackerModal.spec.jsx` (full round trip: no `z-[1200]` node, tap → card + guide line, card commit, re-tap gesture, empty-day add flow, X retention), `BodyFatTrackerModal.spec.jsx`, `StepTrackerModal.spec.jsx` (read-only, empty slots not selectable) and `RollingEnergyBalanceModal.spec.jsx` (store-seeded ledger, untracked day not selectable).
...
102. **Tracker detail is one shared, fixed-slot card — never a positioned tooltip:** the Weight / Body Fat / Step / Rolling Balance modals all render `TrackerSelectionCard` (`common/TrackerSelectionCard.jsx`) pinned inside their `relative` graph container (`absolute top-2`, `flex justify-center`, y-axis inset passed via `className`), always mounted and crossfaded, with the selected slot tied to it by a vertical guide line drawn inside the chart SVG. Do **not** reintroduce measured positioning (`getBoundingClientRect()`/`scrollLeft`), a `fixed z-[1200]` node outside `ModalShell`, an outside-click `document.pointerdown` dismissal, close-on-pan, or per-modal copies of the card markup: measured geometry is what made the old tooltips drift off their point and forced panning to dismiss them, and `z-[1200]` escapes the `1000 + 2/depth` z-lanes. Keep the selection in render-phase retention state (`selectedDate`/`lastSelectedDate`/`selectionSource`) and keep the X a sibling of the card body so dismissing can never commit an edit.
