 ...
 *
 * Usage:
 *   node scripts/food-db/enrich-nutrients.js [--dir <extract-dir>] [--replace]
 *
 * Default dir: scripts/food-db/fdc-download (Foundation/SR Legacy + optional
 * Branded subfolder). Default output is scripts/food-db/foodDatabase.enriched.sqlite;
 * pass --replace to write the canonical bundled DB (a backup is created first).
 */
...
const CANONICAL_DB_PATH = path.resolve(
  PROJECT_ROOT,
...
const ENRICHED_OUTPUT_PATH = path.resolve(
  PROJECT_ROOT,
  'scripts/food-db/foodDatabase.enriched.sqlite'
);
