...
//
// Ingests raw FDC CSV downloads (scripts/food-db/fdc-download) for Foundation +
// SR Legacy (+ Survey when enabled) and produces the curated sql.js catalog used
// at runtime (src/constants/food/foodDatabase.sqlite). Curation rules live in
// ./config/curation.js; every decision is reported to reports/curation.json.
//
// Usage:
//   node scripts/food-db/build.js                # dry-run: reports only
//   node scripts/food-db/build.js --survey       # include Survey/FNDDS foods
//   node scripts/food-db/build.js --replace      # also replace the bundled DB
//   node scripts/food-db/build.js --write        # write foodDatabase.curated.sqlite
...
const FDC_DOWNLOAD_DIR = path.resolve(
  PROJECT_ROOT,
  'scripts/food-db/fdc-download'
);
...
const CURATED_DB_PATH = path.resolve(
  PROJECT_ROOT,
  'scripts/food-db/foodDatabase.curated.sqlite'
);
...
const main = async () => {
  ...
  if (args.replace) {
    ...
    console.log(`Replaced ${SOURCE_DB_PATH} (backup: ${BACKUP_DB_PATH}).`);
  } else {
